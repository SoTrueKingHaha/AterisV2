...
typedef	uint32_t		item_price_t;						// this is the type that is used to hold currency values for transactions! don't change this without changing the relevant code/databases/etc.
typedef uint8_t		item_transaction_quantity_t;
...
typedef uint32_t	operation_definition_index_t;
typedef uint8_t	war_definition_index_t;
typedef uint8_t	war_side_t;

// Misc typedefs for clarity.
typedef uint32_t	equip_region_mask_t;
...
//-----------------------------------------------------------------------------
typedef int32_t entityquality_t;
...
static const uint32_t kKillEaterEvent_PlayerKill = 0;
...
inline void SetBackpackPosition(uint32_t* pPosition, uint32_t iPackPosition)
{
...
inline bool IsNewPositionFormat(uint32_t iBackendPosition)
{
	return (iBackendPosition & kBackendPosition_NewFormat) != 0;
}

inline bool IsUnacknowledged(uint32_t iBackendPosition)
{
...
inline int ExtractBackpackPositionFromBackend(uint32_t iBackendPosition)
{
...
inline unacknowledged_item_inventory_positions_t GetUnacknowledgedReason(uint32_t iBackendPosition)
{
	return (unacknowledged_item_inventory_positions_t)(iBackendPosition &= ~kBackendPositionMask_FormatFlags);
}

inline uint32_t GetUnacknowledgedPositionFor(unacknowledged_item_inventory_positions_t iMethod)
{
...
const uint8_t	 k_unItemRarity_Any = 0xFF;
const uint8_t	 k_unItemQuality_Any = 0xFF;
